...
public class CiclosJava {
	...
	public static void numeroParImpar(int numero) {
		...
		int contador = 1; //La variable nos servirá para evitar que el ciclo se haga infinito, y llevar la cuenta
		//de en que número va
		for(int i=1; i<numero+1; i++) { 	//i++  <=> i = i+1;
			...
			else { //Si no se cumplió lo anterior entonces ejecuta lo siguiente:
				impar = impar + i; // 0 + 1 + 3 +5 +7 +9 +11
			}
		}
		System.out.println("La suma de números pares hasta "+numero+" es: "+par);
		System.out.println("La suma de números impares hasta "+numero+" es: "+impar);
		...
		do { //Haz: 
			...
			contador = contador +1; //Hace que aumente el contador para que llegue a n. Pasará de 1 a N
		}while(contador<=numero); //Mientras el contador sea menor o igual al número repite el proceso.
